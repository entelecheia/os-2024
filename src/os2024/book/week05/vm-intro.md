...
메모리 관점에서 초기 컴퓨터의 운영체제는 사용자에게 많은 발전을 제공하지 않았습니다. 이 시기의 운영체제는 메모리에(그림에서는 물리주소 0부터) 상주하는 루틴 또는 라이브러리의 집합으로 구성되어 있었습니다.

물리 메모리는 주로 그림에서 볼 수 있는 것과 같이 구성되어 있었습니다. 하나의 실행중인 프로그램, 즉 프로세스는 물리 메모리에서 고유한 영역을 할당받아 실행되었으며, 그 외의 메모리 공간은 다른 용도로 사용되었습니다. 예를 들어, 위 그림에서는 물리 주소 64KB부터 시작하여 이러한 구성이 이루어졌습니다.

```{admonition} 주소 체계
논리 주소와 물리 주소는 컴퓨터 시스템에서 사용되는 두 가지 주소 체계입니다.
- 논리 주소(Logical Address): 프로그램이 직접 참조하는 주소입니다. 프로세스는 자신만의 논리 주소 공간을 가지며, 이 주소는 일반적으로 0부터 시작합니다.
- 물리 주소(Physical Address): 실제 메모리 하드웨어에서의 주소입니다. 논리 주소는 이에 매핑되어 실제로 메모리에 저장됩니다.
```

특별한 가상화 기술은 거의 없었으며, 사용자는 운영체제로부터 큰 기능을 기대하지 않았습니다.

그 당시 운영체제 개발자의 삶은 상대적으로 쉽다고 볼 수 있습니다. 이는 현대의 복잡한 운영체제와는 대조적입니다. 초기 컴퓨터 시대의 운영체제는 단순한 기능만을 제공했고, 사용자와의 상호작용이 제한적이었습니다. 이러한 환경에서 운영체제를 개발하고 관리하는 것은 현재의 기술 수준과는 비교할 수 없을 만큼 간단했을 것으로 예상됩니다.
...
시간이 흘러 컴퓨터의 가격이 높았던 시기에는 한 대의 컴퓨터를 여러 사람이 공유하여 사용하는 것이 일반적이었습니다. 이는 컴퓨터 자원을 더 효율적으로 활용하기 위한 방법으로, 멀티프로그래밍 시대를 열었습니다. 이 시기에는 여러 프로세스가 실행을 기다리며, 운영체제가 이들을 전환하여 실행시키는 방식이 채택되었습니다. 이렇게 함으로써 CPU가 놀지 않고 최대한 활용될 수 있었습니다. 그런데도 컴퓨터의 가격은 여전히 매우 높았습니다. 그래서 많은 사람들은 컴퓨터 자원을 더욱 효율적으로 활용하고, 더 많은 사람들이 동시에 사용할 수 있도록 하는 방법을 찾기 시작했습니다.

이런 시기에 시분할 시스템이 개발되었습니다. 이는 여러 사용자가 동시에 컴퓨터를 사용할 수 있도록 하는 것이 목표였습니다. 이전의 일괄처리방식 컴퓨팅에 비해 시분할 시스템은 사용자들이 더 빠르게 결과를 얻을 수 있도록 했습니다. 또한, 프로그래머들이 개발한 프로그램을 빠르게 실행해 보고 수정하는 것도 가능하게 했습니다. 이러한 요구에 맞추어 대화식 이용의 중요성이 부각되었습니다.
...
각 프로세스는 512KB의 물리 메모리에서 각자 작은 부분을 할당 받았고, A는 실행 중이며 B,C는 준비 큐에서 실행을 기다리고 있습니다. 이는 각 프로세스가 메모리를 공유하지 않고 독립적으로 할당된다는 것을 의미하고, 각 프로세스가 자신의 메모리 영역을 보호받는다는 것을 시사합니다. 이것이 중요한 이유는 한 프로세스가 다른 프로세스의 메모리를 읽거나 쓰는 것을 방지하기 위함입니다.

시분할 시스템에서 여러 프로세스가 동시에 실행될 수 있기 때문에, 각 프로세스가 자신의 메모리 영역을 보호받는 것은 매우 중요합니다. 이렇게 함으로써 한 프로세스가 다른 프로세스의 데이터나 코드에 접근하여 변경하는 등의 악의적인 행위를 방지할 수 있습니다. 따라서 운영체제는 메모리 보호를 위해 각 프로세스에게 할당된 메모리 영역을 엄격하게 관리하고 보호해야 합니다. 이는 시분할 시스템이 보다 안전하고 안정적으로 동작하도록 보장하는 데 중요한 역할을 합니다.

## 주소 공간

사용자가 위험한 행위를 할 수 있음을 염두에 두고, 운영체제는 메모리 개념을 사용하기 쉬운 방식으로 만들어야 합니다.

이 개념이 주소 공간(address space)입니다.

주소 공간은 실행 중인 프로그램이 가정하는 메모리의 모습을 나타냅니다. 이는 운영체제가 메모리를 가상화하는 방법을 이해하는 데 중요한 개념입니다. 주소 공간은 프로그램이 사용하는 메모리 영역을 포함하며, 이를 효율적으로 관리하고 보호하기 위해 운영체제가 필요합니다. 따라서 주소 공간을 이해하는 것은 운영체제의 메모리 관리 방식을 이해하는 핵심 요소 중 하나입니다.
