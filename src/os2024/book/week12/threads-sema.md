...
삽입 연산은 리스트의 상태를 변경하고 (전통적인 임계 영역 보호 방식으로 해결 가능합니다), 검색은 자료 구조를 단순히 읽기만 합니다. 삽입 연산이 없다는 보장만 된다면 다수의 검색 작업을 동시에 수행할 수 있습니다. 이와 같은 경우를 위해 만들어진 락이 _reader-writer_ 락이다
