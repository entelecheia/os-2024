...
무작위성을 이용하면 스케줄러를 단순하나 어느 정도 정확하게 만들 수 있지만, 정확한 비율을 보장할 수 없습니다. 짧은 기간만 실행되는 경우는 더욱 그렇습니다. 이 때문에 Waldspurger는 결정론적 공정 배분 스케줄러인 보폭 스케줄링(stride scheduling)을 고안하였습니다.

보폭 스케줄링은 각 프로세스마다 보폭(stride)이라는 값을 할당합니다. 프로세스가 실행될 때마다 CPU 사용량을 추적하기 위해 "pass" 값이라는 변수를 보폭만큼 증가시킵니다. 이 pass 값과 보폭을 스케줄러는 사용하여 어떤 프로세스를 실행할지 결정합니다.

다음은 Waldspurger가 작성한 의사코드 입니다.
...
1. 각 프로세스에게 초기 보폭을 할당합니다. 일반적으로 각 프로세스에는 우선순위 또는 중요도를 나타내는 값을 할당합니다. 이 값이 보폭이 됩니다.

2. CPU를 사용할 수 있는 프로세스 중 가장 작은 보폭을 갖는 프로세스를 선택합니다. 이 프로세스가 CPU를 사용하게 됩니다.

3. 선택된 프로세스는 CPU를 사용하는 동안 자신의 보폭을 증가시킵니다. 이는 다른 프로세스들이 CPU를 공정하게 사용할 수 있도록 합니다.

4. CPU를 사용하는 동안, 다른 프로세스들은 자신의 보폭을 유지하거나 감소시킵니다. 보폭을 감소시키는 것은 CPU를 더 자주 사용할 수 있도록 하는 것이고, 유지하는 것은 CPU 사용을 유지하는 것입니다.

5. CPU 사용이 끝나면, 이 과정을 반복하여 다음 프로세스를 선택합니다.
```

![stride](https://github.com/chu-aie/os-2024/assets/162019986/02bae397-e96d-4240-a469-dc4674f0a849)

이러한 방식으로 보폭 스케줄링은 CPU 사용을 공정하게 분배하여 모든 프로세스가 고르게 CPU를 사용할 수 있도록 합니다. 이는 프로세스들 간의 우선순위나 중요도를 고려하여 자원을 효율적으로 관리하는 데 도움이 됩니다. 

보폭 스케줄링의 정확도를 고려할 때, 추첨 스케줄링을 사용하는 이유는 추첨 스케줄링은 보폭 스케줄링이 갖지 못한 특별한 장점을 가지고 있습니다. 

이 방식은 작업이 도착할 때마다 새로운 작업의 상태 정보를 고려해 스케줄링을 수행합니다. 예를 들어, 보폭 스케줄링에서는 새로운 작업이 도착하면 이 작업의 보폭을 어떻게 설정해야 하는지를 결정해야 합니다. 이때, 보폭을 0으로 설정한다면 해당 작업이 CPU를 독점할 수 있습니다. 반면에 추첨 스케줄링에서는 상태 정보를 유지하고, 새로운 작업이 도착하면 그 작업이 얼마만큼의 추첨권을 가지고 있는지를 고려하여 스케줄링을 수행합니다. 따라서 새로운 작업을 추가할 때는 단순히 새로운 작업이 가진 추첨권의 개수와 전체 추첨권의 개수를 갱신하면 되므로, 새로운 작업을 추가하는 과정이 간편합니다.
